...
class StudentClass:
    def __init__(self, root):
        ...
        self.CourseTable.heading("gender", text="Gerder")
        ...
        try:
            if self.var_roll.get() == "":
                ...
                if row is None:
                    ...
                    self.show()  # Refresh the Treeview to show updated data
        except Exception as ex:
            messagebox.showerror("Error", f"Error due to {str(ex)}", parent=self.root)
        finally:
            con.close()
    ...
    def fetch_course(self):
        ...
        try:
            ...
            if rows:  # Check if rows is not empty
                for row in rows:
                    self.course_list.append(row[0])  # Append the course name to the list
        except Exception as ex:
            messagebox.showerror("Error", f"Error due to {str(ex)}", parent=self.root)
        finally:
            con.close()
