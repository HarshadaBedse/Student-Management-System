...
class resultClass:
    def __init__(self, root):
        ...
        lbl_select=Label(self.root,text=" Select Student",font=("goudy old style",20,"bold"),bg="white").place(x=45,y=100)
        ...
        lbl_marks=Label(self.root,text="Mark's Obtained",font=("goudy old style",20,"bold"),bg="white").place(x=50,y=280)
        lbl_full_marks=Label(self.root,text="Full Mark's",font=("goudy old style",20,"bold"),bg="white").place(x=50,y=340)
        ...
        btn_search = Button(self.root, text="Search", font=("goudy old style", 10, "bold"), bg="#4caf50", fg="white", cursor="hand2",command=self.search).place(x=500, y=100, width=100)
        ...
        txt_full_marks = Entry(self.root, textvariable=self.var_full_marks, font=("goudy old style", 20, "bold"), bg="lightblue").place(x=280, y=340, width=320)


        ################# button ##############################3

        btn_add=Button(self.root,text="Submit",font=("goudy old style",15,),bg="#2196f3",activebackground="lightgreen",cursor="hand2",command=self.add).place(x=300,y=420,width=120,height=35)
        btn_add=Button(self.root,text="Clear",font=("goudy old style",15,),bg="#2196f3",activebackground="lightgreen",cursor="hand2",command=self.clear).place(x=430,y=420,width=120,height=35)
        

        ##content_window#########

        self.bg_img=Image.open("project/images/result.jpgh")
    ...
    def fetch_roll(self):
        ...
        try:
            ...
            if rows:  # Check if rows is not empty
                for row in rows:
                    self.roll_list.append(row[0])  # Append the course name to the list
    ...
    def search(self):
            ...
            try:
                    ...
                    if row != None:
                        ...
                        messagebox.showerror("Info", "No record found", parent=self.root)
            except Exception as ex:
