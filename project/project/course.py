...
class courseClass:
    def __init__(self, root):
        ...
        lbl_search = Label(self.root, text="Search by Course Name:", font=("goudy old style", 15, "bold"), bg="white").place(x=720, y=60)
        self.var_search = StringVar()
        txt_search = Entry(self.root, textvariable=self.var_search, font=("goudy old style", 15), bg="lightyellow").place(x=920, y=60, width=150)
        btn_search = Button(self.root, text="Search", font=("goudy old style", 15, "bold"), bg="#4caf50", fg="white", cursor="hand2", command=self.search).place(x=1080, y=60, width=100)
    ...
    def delete(self):
        ...
        try:
            if self.var_course.get() == "":
                messagebox.showerror("error","course name should be requried",parent=self.root)
            else:
                ...
                if row==None:
                  ...
                  if op==True:
                        ...
                        con.commit()
                messagebox.showinfo("delete","course deleted successfuly",parent=self.root)
                self.clear()
    ...
    def add(self):
        ...
        try:
            if self.var_course.get() == "":
                messagebox.showerror("error","course name should be requride",parent=self.root)
            else:
                cur.execute("select * from course where name =?",(self.var_course.get(),))
                row=cur.fetchone()
                print(row)
